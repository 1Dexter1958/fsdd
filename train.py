...
# Force deterministic for cudnn (may reduce speed)
torch.backends.cudnn.deterministic = True
torch.backends.cudnn.benchmark = False

# For reproducible splits and loader shuffle (PyTorch >=1.8)
...
class Dataset(Dataset):
    ...
    def __getitem__(self, index: int) -> Tuple[torch.Tensor, int]:
        ...
        if self.transform:
            ...
            mel_spec_transform = MelSpectrogram(sample_rate=8000, n_fft=256, hop_length=128, n_mels=32)
...
class SpeechTransformer(nn.Module):
    def __init__(self, num_classes=10, d_model=32, nhead=4, num_layers=4, classifier_type="olsgru"):
        super(SpeechTransformer, self).__init__()
        self.d_model = d_model
        self.positional_encoding = PositionalEncoding(d_model)
        # input projection (mel_bins -> d_model). If mel_bins==d_model this is identity-like.
        self.input_layer = nn.Linear(d_model, d_model)  # safe identity mapping if sizes equal

        # choose classifier
        classifier_type = classifier_type.lower()
        if classifier_type == "gru":
            self.classifier = GRU(d_model, 64, num_classes)
        elif classifier_type == "sgru":
            self.classifier = SGRU(d_model, 64, num_classes)
        elif classifier_type == "lsgru":
            self.classifier = LSGRU(d_model, 64, num_classes)
        else:  # default
            self.classifier = OLSGRU(d_model, 64, num_classes)

        # final linear layer: map d_model -> num_classes
        self.fc = nn.Linear(d_model, num_classes)

    def forward(self, x):
        # x: (batch, mel_bins, time) -> (batch, time, mel_bins)
        x = x.transpose(1, 2)
        # if mel_bins != d_model, map to d_model. Using input_layer as safe mapping.
        x = self.input_layer(x)
        x = self.positional_encoding(x)
        out = self.classifier(x)  # classifier returns either (batch, num_classes) or (batch,time,d_model)
        if out.ndim == 3:
            out = out.mean(dim=1)  # aggregate time -> (batch, d_model)
        # ensure out has shape (batch, d_model)
        if out.size(1) != self.fc.in_features:
            # if dimension mismatches, project/truncate/pad (unlikely here)
            out = out[:, : self.fc.in_features]
...
# -------------------------
def run_train_and_export(audio_dir: str, save_dir: str, epochs: int = 30):
    os.makedirs(save_dir, exist_ok=True)
    dataset = Dataset(audio_dir, transform=True)

    train_size = int(0.9 * len(dataset))
    test_size = len(dataset) - train_size
    train_dataset, test_dataset = random_split(dataset, [train_size, test_size], generator=GLOBAL_GENERATOR)
    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            for inputs, labels in test_loader:
                inputs, labels = inputs.to(device), labels.to(device)
                outputs = model(inputs)  # now model returns logits (batch, num_classes)
                loss = criterion(outputs, labels)
                val_loss += loss.item()
